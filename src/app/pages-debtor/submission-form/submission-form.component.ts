...
export class SubmissionFormComponent {
  ...
  saveSubmission(): any {
    ...
    if (
      this.umkmType === 'mikro' &&
      (newSubmission.loanAmount < 1000000 || newSubmission.loanAmount > 3000000)
    )
      return Swal.fire(
    ...
    if (
      this.umkmType === 'kecil' &&
      (newSubmission.loanAmount < 5000000 ||
        newSubmission.loanAmount > 20000000)
    )
      return Swal.fire(
        'Your Umkm type is small! input amount 5.000.000 - 20.000.000'
      );
    if (
      this.umkmType === 'menengah' &&
      (newSubmission.loanAmount < 10000000 ||
        newSubmission.loanAmount > 50000000)
    )
      return Swal.fire(
